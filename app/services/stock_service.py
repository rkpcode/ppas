...
- "unit_price": float or null (price per unit/strip in rupees)
...
Example output:
[{"medicine_name": "Dolo 650", "generic_name": "Paracetamol", "manufacturer": "Micro Labs", "batch_number": "DL2401", "quantity": 50, "unit_price": 25.5, "expiry_date": "2026-06-01"}]
...
The user is adding new stock to their pharmacy. Extract:
- "medicine_name": string (name of medicine)
- "generic_name": string or null
- "batch_number": string or null
- "quantity": integer (number of strips/units, default 1)
...
Example output:
{"medicine_name": "Dolo 650", "generic_name": null, "batch_number": "DL2401", "quantity": 50, "unit_price": 25.5, "expiry_date": "2026-06-01"}
"""
...
def parse_stock_voice(audio_bytes: Optional[bytes] = None, text: Optional[str] = None) -> Dict[str, Any]:
    """Send voice/text to Gemini and extract stock entry details."""
    model = _get_gemini_model()
    ...
    try:
        response = model.generate_content([STOCK_VOICE_PROMPT, content_parts])
        content = _clean_json_response(response.text)
        return json.loads(content)
    except Exception as e:
        logger.error(f"Stock voice parse failed: {e}")
        return {
            "medicine_name": text if text else "Unknown",
            "quantity": 1,
            "unit_price": None,
            "batch_number": None,
            "expiry_date": None,
